...
export default function DashboardDrawer({ open, onClose, onOpen }) {
  ...
  const [showDeleted, setShowDelete] = useState(getOption("showDeleted", v => v === "true"));
  ...
  return (
    <SwipeableDrawer open={open} onClose={onClose} onOpen={onOpen}>
      <List subhread={<ListSubheader>Preferences</ListSubheader>}>
        ...
        <ListItem button onClick={() => setProgress(!progress)}>
          ...
          <ListItemText primary="Progress" secondary={progress ? "On" : "Off"} />
        </ListItem>
        <ListItem button onClick={() => setShowDelete(!showDeleted)}>
          <ListItemIcon>
            <Switch checked={showDeleted} />
          </ListItemIcon>
          <ListItemText primary="Show deleted" secondary={showDeleted ? "On" : "Off"} />
        </ListItem>
      </List>
      {LISTS.map(({ header, items }, listIndex) => (
        <List key={listIndex} subhread={<ListSubheader>{header}</ListSubheader>}>
